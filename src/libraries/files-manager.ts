...
export function parsePlainTextToJson(plainText: string): Session {
  ...
  let sessionStart = '';
  let sessionEnd = '';
  const sessionDuration = sessionDurationMatch ? sessionDurationMatch[1].replace(/,/g, '') : '00:00h';
  if (sessionTimes) {
  ...
  const xpPerHour = xpPerHourMatch ? Number(xpPerHourMatch[1].replace(/,/g, '')) : 0;
  ...
  const damagePerHour = damagePerHourMatch ? Number(damagePerHourMatch[1].replace(/,/g, '')) : 0;
  ...
  const healingPerHour = healingPerHourMatch ? Number(healingPerHourMatch[1].replace(/,/g, '')) : 0;

  // Extract the killed monsters and looted items
  const killedMonstersMatch = plainText.match(/Killed Monsters:(.*)Looted Items:/s);
...
export function standarizeJson(jsonText: string): Session {
  ...
  try {
    jsonData = JSON.parse(jsonText, (key, value) => {
      ...
      if (key === 'Session end' || key === 'Session start' || key === 'Session length') {
